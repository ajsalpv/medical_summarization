...
def model_ans():
    ...
    w_mdl='dmis-lab/biobert-v1.1'
    # w_mdl='sentence-transformers/LaBSE'
...
def get_pdf_texts_db( uploaded_file):
...
def get_ans(query):
    ...
    retriever=pdf_db.as_retriever(search_type='similarity_score_threshold', search_kwargs={"score_threshold": 0.5})
    docs = retriever.get_relevant_documents(query)
    print('type of the docs is : ',type(docs),'length of the docs: ',len(docs))
    return docs


def get_web_text_db(website_url):
    text=""
    ...
    print('embeddign')
    web_db = FAISS.from_texts(texts, embeddings)
    print('vector')
    return web_db,embeddings



def get_ans_web(query):
    # texts=texts[0]
    
    embedding_vector = embeddings.embed_query(query)
    docs=web_db.similarity_search_by_vector(embedding_vector)
    print('type of the docs is : ',type(docs),'length of the docs: ',len(docs))
    return docs
...
def app():
    ...
    if uploaded_file:
        pdf_db=get_pdf_texts_db(uploaded_file)
    elif website_url:
        web_db,embeddings=get_web_text_db(website_url)
