...
def main():
    ...
    if uploaded_file is not None or website_url:
            ...
            if st.button("ChatBot"):
                ...
                # user_query=st.text_input('sldgjla')
                # print('sljkg',user_query)
                # if user_query:
                ...
                #     if uploaded_file is not None:
                #         chunks = text_splitter.split_text(text=st.session_state.user_query)
                #     elif website_url:
                #         chunks = text_splitter.split_text(text=get_text_from_website(website_url))

                #     embeddings = HuggingFaceInstructEmbeddings(model_name="hkunlp/instructor-large")
                #     vectorstore = FAISS.from_texts(texts=chunks, embedding=embeddings)
                #     result = RetrievalQAWithSourcesChain.from_llm(llm=llm, retriever=vectorstore.as_retriever())({"question": user_query}, return_only_outputs=True)
                #     st.header("Answer")
